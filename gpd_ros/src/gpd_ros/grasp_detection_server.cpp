...
bool GraspDetectionServer::detectGrasps(gpd_ros::detect_grasps::Request& req, gpd_ros::detect_grasps::Response& res)
{
  ...
  if (grasps.size() > 0)
  {
    ...
    // ROS_INFO_STREAM("Detected " << selected_grasps_msg.grasps.position.x << " highest-scoring grasps.");
    // ROS_INFO_STREAM("Detected " << selected_grasps_msg.grasps.sample << " highest-scoring grasps.");
    return true;
  }
