...
public:
    ...
    : arm_planner_(config)    
    {
        ...
        planning_server_ =  nh.advertiseService("/plan_right_arm_motion", &RosPlanningInterface::calculation, this);
    }
private:
    bool calculation(arm_motion_planner::plan_arm_motion::Request &req, arm_motion_planner::plan_arm_motion::Response &res)
    {
        ...
        arm_planner_.compute(res.torso_joint_trajectory, res.arm_joint_trajectory);
        if (res.arm_joint_trajectory.points.empty())
        {
...
int main(int argc, char **argv)
{
    ...
    ros::init(argc, argv, "RightArmPlannerServer");
    ros::NodeHandle nh("~");
    ...
    RosPlanningInterface right_arm_planning_interface(nh, config);

    ROS_INFO("ready right arm planner server!");
